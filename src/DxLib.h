...
// ＤＸライブラリのバージョン
#define DXLIB_VERSION 0x222f
#define DXLIB_VERSION_STR "2.22f"
...
// VisualC++の標準メモリデバッグ機能を使用する場合はコメントを外し、
...
// 再生時メッセージコード
#define PLAY_MESAGGE								(WM_USER + 290)
...
extern	int CheckHitKeyAll( void ) ;												// 全キーの押下状態を取得
...
extern	int		SetDDrawUseGuid( GUID FAR *Guid ) ;							// ＤｉｒｅｃｔＤｒａｗが使用するＧＵＤＩを設定する
...
extern	void					*GetPixelFormat( void ) ;		/*戻り値を DDPIXELFORMAT * にキャストして下さい*/			// ピクセルフォーマットの取得
...
extern	GUID FAR *				GetDirectDrawDeviceGUID( int Number ) ;								// 有効な DirectDraw デバイスの GUID を取得する
...
// 色情報取得関係
extern	int		ColorKaiseki( void *PixelData, LPCOLORDATA ColorData ) ;									// 色ビット情報解析
...
extern	int		GetColor3( LPCOLORDATA ColorData, int Red, int Green, int Blue, int Alpha = 255 ) ;			// ３原色値から指定のピクセルフォーマットに対応した色データ値を得る
extern	int		GetColor4( LPCOLORDATA DestColorData, LPCOLORDATA SrcColorData, int SrcColor ) ;			// ２つのカラーフォーマット間のデータ変換を行った情報を得る 
extern	int		GetColor5( LPCOLORDATA ColorData, int Color, int *Red, int *Green, int *Blue, int *Alpha = NULL ) ;	// 指定カラーフォーマットに対応した色データ値から個々の３原色データを抜き出す
extern	int		CreatePaletteColorData( LPCOLORDATA ColorDataBuf ) ;										// パレットカラーのカラー情報を構築する
extern	int		CreateXRGB8ColorData( LPCOLORDATA ColorDataBuf ) ;											// ＸＲＧＢ８カラーのカラー情報を構築する
extern	int		CreateARGB8ColorData( LPCOLORDATA ColorDataBuf ) ;											// ＡＲＧＢ８カラーのカラー情報を構築する
extern	int		CreateFullColorData( LPCOLORDATA ColorDataBuf ) ;											// フルカラーＤＩＢのカラー情報を構築する
extern	int		CreateGrayColorData( LPCOLORDATA ColorDataBuf ) ;											// グレースケールのカラー情報を構築する
extern	int		CreateColorData( LPCOLORDATA ColorDataBuf, int ColorBitDepth,
...
extern	void	SetColorDataNoneMask( LPCOLORDATA ColorData ) ;												// NoneMask 以外の要素を埋めた COLORDATA 構造体の情報を元に NoneMask をセットする
extern	int		CmpColorData( LPCOLORDATA ColorData1, LPCOLORDATA ColorData2 ) ;							// 二つのカラーデータが等しいかどうか調べる( TRUE:等しい  FALSE:等しくない )
...
extern	VERTEX_2D *GetVertexBuffer( void ) ;							// 追加頂点バッファの頂点追加アドレスを得る
...
// その他補助関数
extern void			*GetTexPixelFormat( int AlphaCh, int AlphaTest, int ColorBitDepth, int DrawValid = FALSE ) ;		/*戻り値を DDPIXELFORMAT * にキャストして下さい*/	// ピクセルフォーマットを得る
extern LPCOLORDATA	GetTexColorData( int AlphaCh, int AlphaTest, int ColorBitDepth, int DrawValid = FALSE ) ;			// カラーデータを得る
extern void			*GetTexPixelFormat( IMAGEFORMATDESC *Format ) ;	/*戻り値を DDPIXELFORMAT * にキャストして下さい*/	// フォーマットに基づいたカラーフォーマットを得る
extern LPCOLORDATA	GetTexColorData( IMAGEFORMATDESC *Format ) ;														// フォーマットに基づいたカラーデータを得る
extern void			*GetTexPixelFormat( int FormatIndex ) ;	 /*戻り値を DDPIXELFORMAT * にキャストして下さい*/			// 指定のフォーマットインデックスのカラーフォーマットを得る
extern LPCOLORDATA	GetTexColorData( int FormatIndex ) ;																// 指定のフォーマットインデックスのカラーデータを得る
extern int			GetTexFormatIndex( IMAGEFORMATDESC *Format ) ;														// テクスチャフォーマットのインデックスを得る
extern void			*GetZBufferPixelFormat( int BitDepth ) ;	/*戻り値を DDPIXELFORMAT * にキャストして下さい*/		// 指定のビット深度のＺバッファーのピクセルフォーマットを得る( 現在のところ 16 ビットのみ )
...
// グラフィック制御関係関数
extern	int			MakeGraph( int SizeX, int SizeY ) ;																															// 空のグラフィックを作成
extern	int			MakeGraph( int SizeX, int SizeY, int NotUse3DFlag ) ;																										// 空のグラフィックを作成
...
extern	int			LoadGraph( const char *FileName ) ;																															// 画像ファイルのメモリへの読みこみ
extern	int			LoadGraph( const char *FileName, int NotUse3DFlag ) ;																										// 画像ファイルのメモリへの読みこみ
...
extern	int			LoadReverseGraph( const char *FileName ) ;																													// 画像ファイルのメモリへの反転読み込み
extern	int			LoadReverseGraph( const char *FileName, int NotUse3DFlag ) ;																								// 画像ファイルのメモリへの反転読み込み
extern	int			LoadDivGraph( const char *FileName, int AllNum, int XNum, int YNum, int XSize, int YSize, int *HandleBuf ) ;												// 画像の分割読みこみ
extern	int			LoadDivGraph( const char *FileName, int AllNum, int XNum, int YNum, int XSize, int YSize, int *HandleBuf, int NotUse3DFlag ) ;								// 画像の分割読みこみ
extern	int			LoadReverseDivGraph( const char *FileName, int AllNum, int XNum, int YNum, int XSize, int YSize, int *HandleBuf ) ;											// 画像の反転分割読みこみ
extern	int			LoadReverseDivGraph( const char *FileName, int AllNum, int XNum, int YNum, int XSize, int YSize, int *HandleBuf, int NotUse3DFlag ) ;						// 画像の反転分割読みこみ
#ifndef DX_NON_MOVIE
...
extern	int			DrawLine( int x1, int y1, int x2, int y2, int Color ) ;																				// 線を描画
...
extern	int			DrawCircle( int x, int y, int r, int Color, int FillFlag ) ;																		// 円を描く
extern	int			DrawCircle( int x, int y, int r, int Color ) ;																						// 円を描く
...
extern	int			SetRestoreShredPoint( void ( *ShredPoint )( void ) ) ;				// グラフィック復元関数の登録
...
extern	int			DrawMaskToDirectData( int DrawPointX, int DrawPointY, void *DestBufP, int DestPitch, 
											int SrcWidth, int SrcHeight, void *SrcData, int TransMode  ) ;					// マスクパターンの展開
...
extern	int		ReleaseBaseImage(  BASEIMAGE *GraphImage ) ;																																																	// 基本イメージデータの後始末を行う
